...
function initTower(x, y, z, index){
  ...
  loader.load("models/arrow.obj",
        function ( obj ) {
          geometry = new THREE.CylinderGeometry(0.6, 1, 30, 32)
          mesh  = new Physijs.CylinderMesh(geometry, new THREE.MeshLambertMaterial({color: 0xefedba}), 0)
          plane = new Physijs.PlaneMesh(new THREE.PlaneGeometry(8, 8, 1), new THREE.MeshLambertMaterial({color: 0xefedba}), 0)
          plane.rotateX(Math.PI/2)
          plane.translateZ(-15.5)
          mesh.add(plane)
          obj.scale.x = 2
          obj.scale.y = 2
          obj.scale.z = 2
